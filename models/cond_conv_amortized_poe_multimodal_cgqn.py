...
class CondConv2d(nn.Module):
    def __init__(self,
        ...
        self.idenfitier_size = idenfitier_size
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding

        self.weight = Parameter(torch.Tensor(
            idenfitier_size, output_size, input_size, kernel_size, kernel_size))
        self.bias = Parameter(torch.Tensor(
            idenfitier_size, output_size))
        self.reset_parameters()

    def reset_parameters(self):
        for i in range(self.idenfitier_size):
    ...
    def forward(self, input, identifier):
        weight = self.weight[identifier]
        bias   = self.bias[identifier]
        output = F.conv2d(input, weight, bias, stride=self.stride, padding=self.padding)
        return output
...
class CGQNConvDraw(nn.Module):
    def __init__(self,
        ...
        #self.cond_enc_q = CondConv2d(nc_context*2, nc_context*2, kernel_size=5, padding=2)
        #self.cond_enc_p = CondConv2d(nc_context,   nc_context,   kernel_size=5, padding=2)
        self.cond_enc = CondConv2d(nc_context, nc_context, self.num_multimodalities, kernel_size=5, padding=2)
        ...
        for i in range(num_steps if num_steps is not None else self.num_steps):
            ...
            if not init_input_q:
                for m in range(self.num_multimodalities):
                    ...
                    reps_context = self.cond_enc(reps_context, m)
                    ...
                    reps_target = self.cond_enc(reps_target, m)
