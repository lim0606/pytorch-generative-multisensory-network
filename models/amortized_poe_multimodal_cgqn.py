...
class CGQNConvDraw(nn.Module):
    ...
    def forward(self,
        ...
        for m in range(self.num_multimodalities):
            states_ps  += [self.rnn_p.init_state(num_episodes, [self.z_height, self.z_width])] # note: for training initial state, you need to change this (as the init_state should differ from each modality
            states_qs  += [self.rnn_q.init_state(num_episodes, [self.z_height, self.z_width])] # note: for training initial state, you need to change this (as the init_state should differ from each modality
        ...
        for i in range(num_steps if num_steps is not None else self.num_steps):
            ...
            for m in range(self.num_multimodalities):
                ...
                # forward posterior
                means_q, logvars_q, hiddens_q, states_q = self.rnn_q(input_q, states_q, hiddens_p)
            ...
            for m in range(self.num_multimodalities):
                ## mod identifier
                #mod_identifier = self.modality_identifier[m:m+1, :, :, :].expand(num_episodes, self.num_multimodalities, self.z_height, self.z_height)
                ...
                if not init_input_p:
                    ...
                    #reps_context = pad_sequence(reps_context, context_sizes)
                    #reps_context = torch.sum(reps_context, dim=1)
                    #reps_context = reps_context.view(-1, self.nc_context, self.z_height, self.z_height)
                ...
                # forward prior
                _, means_p, logvars_p, hiddens_p, states_p = self.rnn_p(input_p, states_p, latents_q=zs)
    ...
    def generate(self,
        ...
        for i in range(num_steps if num_steps is not None else self.num_steps):
            ...
            for m in range(self.num_multimodalities):
                ...
                # forward prior (rnn)
                hiddens_p, states_p = self.rnn_p.forward_rnn(input_p, states_p, zs)
    ...
    def predict(self,
        ...
        # additional loss info
        info['likelihood'] = loss_likelihood.detach() if type(loss_likelihood) is not int else 0
        info['kl'] = loss_kl.detach()
    ...
    #def _forward_img_renderer(self, img_queries, latents, num_episodes, img_batch_sizes, img_target_indices=[]):
    #    ''' forward image renderer '''
    #    # generate recon (likelihood)
    #    # embed query
    #    emb_img_queries = [self.query_encoders[0](img_query) for img_query in img_queries]

    #    # concat z
    #    z = torch.cat(latents, dim=1)

    #    # run renderer (likelihood)
    #    #img_mean_recon, img_logvar_recon = self.img_renderer(z, emb_img_queries, img_batch_sizes)
    #    img_mean_recon = self.renderers[0](z, emb_img_queries, img_batch_sizes, img_target_indices, num_episodes)
    #    return img_mean_recon#, img_logvar_recon

    #def _forward_hpt_renderer(self, hpt_queries, latents, num_episodes, hpt_batch_sizes, hpt_target_indices=[]):
    #    ''' forward haptic renderer '''
    #    # generate recon (likelihood)
    #    # embed query
    #    emb_hpt_queries = [self.query_encoders[1](hpt_query) for hpt_query in hpt_queries]

    #    # concat z
    #    z = torch.cat(latents, dim=1)

    #    # run renderer (likelihood)
    #    #hpt_mean_recon, logvar_recon = self.hpt_renderer(new_z, emb_hpt_query)
    #    hpt_mean_recon = self.renderers[1](z, emb_hpt_queries, hpt_batch_sizes, hpt_target_indices, num_episodes)
    #    return hpt_mean_recon#, hpt_logvar_recon
...
class CGQN(nn.Module):
        ...
        for dim in dims:
            ...
            if mtype == 'image':
                ...
                repnets += [HapticContextNetwork(
                        ...
                        nchannels=channels,
                        nhidden = 512, #1024,
                        nz=self.convdraw.nc_context*self.convdraw.z_height*self.convdraw.z_height,
                        train_init_representation=True,
                        )]
            else:
    ...
    def forward(self, contexts, targets, num_steps=None, beta=1.0, std=1.0, is_grayscale=False):
        ...
        for i in range(self.num_multimodalities):
            ...
            mod_reps_c = [mod_rep_c.view(-1, self.convdraw.nc_context, self.convdraw.z_height, self.convdraw.z_height) for mod_rep_c in mod_reps_c]
        ...
        for i in range(self.num_multimodalities):
            ...
            mod_reps_t = [mod_rep_t.view(-1, self.convdraw.nc_context, self.convdraw.z_height, self.convdraw.z_height) for mod_rep_t in mod_reps_t]
        ...
        (mean_recons,
         ...
         info) = self.convdraw(
                 ...
                 reps_context_sizes_pairs_t,
                 #reps_t, context_sizes_t,
                 data_queries_target_indices_batch_sizes,
                 #img_target = images,
                 #img_queries  = img_queries,
                 #img_batch_sizes = img_batch_sizes,
                 #img_target_indices = img_target_indices,
                 #hpt_target = haptics,
                 #hpt_queries  = hpt_queries,
                 #hpt_batch_sizes = hpt_batch_sizes,
                 #hpt_target_indices = hpt_target_indices,
                 num_steps=num_steps, beta=beta, std=std, is_grayscale=is_grayscale)
        #return img_mean_recon, hpt_mean_recon, latent, loss, info
    ...
    def generate(self, contexts, queries, num_steps=None, is_grayscale=False):
        ...
        mean_recons, latent = self.convdraw.generate(
                reps_context_sizes_pairs_c,
                #reps_c, context_sizes_c,
                queries_batch_sizes,
