...
from models.reparam import NormalDistributionConv3d
...
class ConvDrawEncoderCell(nn.Module):
    def __init__(self,
        ...
        self.reparam = NormalDistributionConv3d(hidden_size, nz,
                                                kernel_size=(1, kernel_size, kernel_size),
                                                stride=(1, stride, stride),
                                                padding=(0, padding, padding),
                                                )
...
class ConvDrawDecoderCell(nn.Module):
    def __init__(self,
        ...
        self.reparam_p = NormalDistributionConv3d(reparam_input_size, nz,
                                                  kernel_size=(1, kernel_size, kernel_size),
                                                  stride=(1, stride, stride),
                                                  padding=(0, padding, padding),
                                                  )
    ...
    def forward_prob(self, prev_state_p, higher_hiddens=[]):
        # unpack hidden_p
        ...
        return mean_p, logvar_p

    def forward_rnn(self, prev_state_p, input=None, z=None, higher_hiddens=[]):
        ## unpack hidden_p
        #hidden_p = prev_state_p[0]
    ...
    def forward(self, prev_state_p, input=None, z=None, higher_hiddens=[]):
        ...
        # forward prob
        mean_p, logvar_p = self.forward_prob(prev_state_p, input, higher_hiddens)
        ...
        # forward rnn step
        hidden_p, new_state_p = self.forward_rnn(prev_state_p, input, z, higher_hiddens)
...
class StackedConvDrawDecoderCell(nn.Module):
    ...
    def forward_prob(self, prev_states):
        # init new_states
        new_means   = []
        new_logvars = []
        new_hiddens = []
        ...
        for j in range(self.num_layers-1, 0, -1):
            # forward prob
            mean_p, logvar_p = self.rnns[j].forward_prob(
                    prev_states[j], higher_hiddens=new_hiddens)
            new_means   += [mean_p]
            new_logvars += [logvar_p]
            new_hiddens += [prev_states[j][0]]

        # forward rnn (last layer)
        mean_p, logvar_p = self.rnns[0].forward_prob(
                prev_states[0], higher_hiddens=new_hiddens)
        new_means   += [mean_p]
        new_logvars += [logvar_p]
        ...
        return new_means, new_logvars

    def forward_rnn(self, context, prev_states, latents_q):
        ''' forward stacked rnn one time step
        Input:
            input: batch_size x input_size
            prev_states: a list with length = number of layers (of stacked RNNs)
        ...
        for j in range(self.num_layers-1, 0, -1):
            ...
            # forward rnn
            hidden_p, state = self.rnns[j].forward_rnn(
        ...
        # forward rnn (last layer)
        hidden_p, state = self.rnns[0].forward_rnn(
    ...
    def forward(self, context, prev_states, latents_q=None):
        ...
        '''
        # forward prob
        new_means, new_logvars = self.forward_prob(prev_states)

        # sample z
        if latents_q is None:
            new_latents = self.sample(new_means, new_logvars)
        else:
            new_latents = latents_q

        # forward rnn step
        new_hiddens, new_states = self.forward_rnn(context, prev_states, new_latents)
