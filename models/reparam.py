...
class MixtureOfGaussiansDistribution(nn.Module):
    def __init__(self, num_gaussians=1, nonlinearity=None):
        super().__init__()
    ...
    def sample(self, pi, mus, logvars):
        # init
        pi = F.softmax(pi, dim=1)
        rv = Categorical(pi)
        mus = mus.chunk(self.num_gaussians, 1)
        logvars = logvars.chunk(self.num_gaussians, 1)
    ...
    def forward(self, input):
        pi = self.pi_fn(input)
        mus = self.mean_fn(input)
        logvars = self.clip_logvar(self.logvar_fn(input))
        return pi, mus, logvars

class MixtureOfGaussiansDistributionLinear(MixtureOfGaussiansDistribution):
    def __init__(self, input_size, output_size, num_gaussians, nonlinearity=None):
        super().__init__(num_gaussians=num_gaussians,
        ...
        # define net
        self.pi_fn = nn.Linear(input_size, num_gaussians)
        self.mean_fn = nn.Linear(input_size, output_size*num_gaussians)
        self.logvar_fn = nn.Linear(input_size, output_size*num_gaussians)
...
class NormalDistributionConv3d(NormalDistribution):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True, nonlinearity=None):
        super().__init__(nonlinearity=nonlinearity)

        # define net
        self.mean_fn = nn.Conv3d(in_channels, out_channels, kernel_size, stride, padding, bias=bias)
        self.logvar_fn = nn.Conv3d(in_channels, out_channels, kernel_size, stride, padding, bias=bias)
...
class BernoulliDistributionConv2d(BernoulliDistribution):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, hard=False):
        super(BernoulliDistributionConv2d, self).__init__(hard=hard)

        # define net
        self.logit_fn = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding) 
