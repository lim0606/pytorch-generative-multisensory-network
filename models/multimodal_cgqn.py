...
class CGQNConvDraw(nn.Module):
    ...
    def forward(self,
        ...
        # init states
        states_p = self.rnn_p.init_state(num_episodes, [self.z_height, self.z_width])
        states_q = self.rnn_q.init_state(num_episodes, [self.z_height, self.z_width])
    ...
    def generate(self,
        ...
        for i in range(num_steps if num_steps is not None else self.num_steps):
            ...
            # forward prior
            zs, means_p, logvars_p, hiddens_p, states_p = self.rnn_p(input_p, states_p)
...
class CGQN(nn.Module):
        ...
        for dim in dims:
            ...
            if mtype == 'image':
                ...
                repnets += [HapticContextNetwork(
                        ...
                        output_height=self.convdraw.z_height,
                        output_width=self.convdraw.z_width,
                        train_init_representation=True,
                        use_instance_norm=self.use_instance_norm,
                        )]
            else:
    ...
    def infogain(self, contexts, targets, num_steps=None, beta=1.0, std=1.0):
        (reps_c, context_sizes_c,
         reps_t, context_sizes_t,
         data_queries_target_indices_batch_sizes) = self._forward(contexts, targets)#, merge_cxt_tgt=True)
    ...
    def generate(self, contexts, queries, num_steps=None, is_grayscale=False):
        reps_c, context_sizes_c = self._generate(contexts)
